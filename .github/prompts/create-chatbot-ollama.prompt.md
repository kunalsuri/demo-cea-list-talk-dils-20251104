...
* Provide complete, ready-to-apply code for:
...
* Include concise comments in the code explaining:

  * The purpose of each file and major component.
  * How the streaming flow works end-to-end (frontend ↔ backend ↔ Ollama).
* Maintain a production-ready, maintainable architecture:

  * Type-safe TypeScript definitions for request/response payloads and message types.
  * Clear separation between UI, business logic, and external API integration.
* Output all files in a clean, structured format, grouped by path (e.g., `frontend/...`, `backend/...`), so they can be directly copied into the existing codebase.
